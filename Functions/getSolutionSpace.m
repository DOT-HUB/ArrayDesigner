function [refpts_10_2p5,refpts_10_2p5_labels,refpts_10_5,refpts_10_5_labels,ten2p5_to_ten5] = getSolutionSpace(pathnameHeadModel,pathnameMSHS)

% This function computes the 10-5 and 10-2.5 locations on the head surface and saves
% them under /Utils/scalpSolutionSpace_10_2p5.txt and /Utils/scalpSolutionSpace_10_5.txt 
% 
% INPUTS: 
%
% pathnameHeadModel         : pathname to the head model folder containing
%                           a .mshs file of the same name. The minimum
%                           contents of the .mshs file is as below:
%
% pathnameMSHS              : path to .mshs file with structure:
...
% refpts_10_2p5_label       :
%
% ten2p5_to_ten5             : Logical index specifying which of the ten 2.5
%                             points are also ten 5.


%Upsample scalp surface mesh if needed (need to find a threshold where this is not needed ###)
...
%Check if tenFive exists, if not, calculate it and save it in to mshs for
%future use as it is part of the file definition.
if ~isfield(mshs,'tenFive')
    [tenFive.positions, tenFive.labels] = DOTHUB_getTen5points(mshs.scalpSurfaceMesh.node,mshs.landmarks);
    mshs.tenFive = tenFive;
    save(pathnameMSHS,'tenFive','-append');
end
refpts_10_5 = mshs.tenFive.positions;
refpts_10_5_labels = mshs.tenFive.labels;
...
refpts_10_5_labels = refpts_10_5_reorder_label;

% Create 10-2.5 coordinates %%%%%%%%%%%%%%%%%
refpts_10_2p5 = refpts_10_5;
refpts_10_2p5_labels = refpts_10_5_labels;
refpts_10_2p5_tmp = refpts_10_5;
refpts_10_2p5_label_tmp = refpts_10_5_labels;
...
for j = 1:size(Left_Coronal_points,2)
    ...
    for i = 1:size(Left_Coronal_points,1)-1
        ...
        ind1 = find(strcmpi(Left_Coronal_points(i,j),refpts_10_2p5_labels));
        ind2 = find(strcmpi(Left_Coronal_points(i+1,j),refpts_10_2p5_labels));
        
        tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
        qpoints(count,:) = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
        ...
        refpts_10_2p5_labels(ind1+1) = strcat(refpts_10_2p5_labels(ind1,1), 'q');
        refpts_10_2p5_labels(ind1+2:ind1+1+length(refpts_10_2p5_tmp(ind1+1:end,:))) = refpts_10_2p5_label_tmp(ind1+1:end);
        
        refpts_10_2p5_tmp =  refpts_10_2p5;
        refpts_10_2p5_label_tmp =  refpts_10_2p5_labels;
...
for j = 1:size(Right_Coronal_points,2)
    ...
    for i = 1:size(Right_Coronal_points,1)-1
        ...
        ind1 = find(strcmpi(Right_Coronal_points(i,j),refpts_10_2p5_labels));
        ind2 = find(strcmpi(Right_Coronal_points(i+1,j),refpts_10_2p5_labels));
        ...
        refpts_10_2p5_labels(ind2) = strcat(refpts_10_2p5_labels(ind2,1), 'q');
        refpts_10_2p5_labels(ind2+1:ind2+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);
...
for j = 1:size(Left_Coronal_points,2)-1
    qpoints = [];
    ind_first_line1 = find(strcmpi(Left_Coronal_points(1,j),refpts_10_2p5_labels));
    ind_first_line2 = find(strcmpi(Left_Coronal_points(1,j+1),refpts_10_2p5_labels));
    
    for i = 1:n
        tmp = refpts_10_2p5(ind_first_line1+(i-1),:) + (refpts_10_2p5(ind_first_line2+(i-1),:)-refpts_10_2p5(ind_first_line1+(i-1),:))./2;
        qpoints(i,:) = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
        
        %Get label for new points
        tmp = Get_10_2p5_label(refpts_10_2p5_labels{ind_first_line1+(i-1)},refpts_10_2p5_labels{ind_first_line2+(i-1)});
        if strcmp('ERR',tmp)
            pause;
        end
        qlabels{i} = tmp;
    ...
    refpts_10_2p5_labels(end+1:end+n) = qlabels;
...
for j = 1:size(Right_Coronal_points,2)-1
    qpoints = [];
    ind_first_line1 = find(strcmpi(strcat(Right_Coronal_points(2,j),'q'),refpts_10_2p5_labels));
    ind_first_line2 = find(strcmpi(strcat(Right_Coronal_points(2,j+1),'q'),refpts_10_2p5_labels));
    ...
    for i = 1:n
        ...
        %Get label for new points
        qlabels{i} = Get_10_2p5_label(refpts_10_2p5_labels{ind_first_line1+(i-1)},refpts_10_2p5_labels{ind_first_line2+(i-1)});
    end
...
for i = 1:length(Midline_Points)-1
    ind1 = find(strcmpi(Midline_Points(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Midline_Points(i+1),refpts_10_2p5_labels));
    
    tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
    qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
    
    %Place new point in sequence in refpts_10_2p5
    refpts_10_2p5(ind1+1,:) = qpoint;
    refpts_10_2p5(ind1+2:ind1+1+length(refpts_10_2p5_tmp(ind2:end,:)),:) = refpts_10_2p5_tmp(ind1+1:end,:);
    
    refpts_10_2p5_labels{ind1+1} = Get_10_2p5_label(refpts_10_2p5_labels{ind1},refpts_10_2p5_labels{ind2});
    refpts_10_2p5_labels(ind1+2:ind1+1+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);
...
for i = 1:length(Lower_Ring_Left_points)-1
    ...
    ind1 = find(strcmpi(Lower_Ring_Left_points(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Lower_Ring_Left_points(i+1),refpts_10_2p5_labels));
    ...
    if ismember(i,cross_points) %If first point (ind1) is a cross point, then point should be index ind2-1
        ...
        if i == 1
            tmp2 = refpts_10_2p5_labels{ind2};
            refpts_10_2p5_labels{ind2} = strcat(tmp2(1:end-1),'q');
            refpts_10_2p5_labels(ind2+1:ind2+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);            
        else
            refpts_10_2p5_labels{ind2} = strcat(refpts_10_2p5_labels{ind2},'q');
            refpts_10_2p5_labels(ind2+1:ind2+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);
        end
    else
        refpts_10_2p5(ind1+1,:) = qpoint;
...
for i = 1:length(Lower_Ring_Right_points)-1
    ind1 = find(strcmpi(Lower_Ring_Right_points(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Lower_Ring_Right_points(i+1),refpts_10_2p5_labels));
...
for i = 1:length(Upper_Ring_Left_points)-1
    ...
    ind1 = find(strcmpi(Upper_Ring_Left_points(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Upper_Ring_Left_points(i+1),refpts_10_2p5_labels));
...
for i = 1:length(Upper_Ring_Right_points)-1
    ind1 = find(strcmpi(Upper_Ring_Right_points(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Upper_Ring_Right_points(i+1),refpts_10_2p5_labels));
...
for i = 1:length(start_end_points_lower_left)
    ind_lower_left(i) = find(strcmpi(start_end_points_lower_left(i),refpts_10_2p5_labels));
    ind_lower_right(i) = find(strcmpi(start_end_points_lower_right(i),refpts_10_2p5_labels));  
    ind_upper_left(i) = find(strcmpi(start_end_points_upper_left(i),refpts_10_2p5_labels));
    ind_upper_right(i) = find(strcmpi(start_end_points_upper_right(i),refpts_10_2p5_labels));
end
...
for i = 1:length(ind_lower_left)
    ...
    refpts_10_2p5_labels{end+1,:} = strcat(refpts_10_2p5_labels{ind1},'q');
end
...
for i = 1:length(occ_line_left)-1
    ind1 = find(strcmpi(occ_line_left(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(occ_line_left(i+1),refpts_10_2p5_labels));
    ...
    if i == 4
        ...
        refpts_10_2p5_labels{ind1+1} = strcat(refpts_10_2p5_labels{ind1},'h');
        refpts_10_2p5_labels(ind1+2:ind1+1+length(refpts_10_2p5_tmp(ind1+1:end,:))) = refpts_10_2p5_label_tmp(ind1+1:end);
    else
        ...
        refpts_10_2p5_labels{ind2} = strcat(refpts_10_2p5_labels{ind1},'h');
        refpts_10_2p5_labels(ind2+1:ind2+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);
...
for i = 1:length(occ_line_right)-1
    ind1 = find(strcmpi(occ_line_right(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(occ_line_right(i+1),refpts_10_2p5_labels));
    ...
    if i == 4
        ...
        refpts_10_2p5_labels{ind1+1} = strcat(refpts_10_2p5_labels{ind2},'h');
        refpts_10_2p5_labels(ind1+2:ind1+1+length(refpts_10_2p5_tmp(ind1+1:end,:))) = refpts_10_2p5_label_tmp(ind1+1:end);
    else
        ...
        refpts_10_2p5_labels{ind2} = strcat(refpts_10_2p5_labels{ind2},'h');
        refpts_10_2p5_labels(ind2+1:ind2+length(refpts_10_2p5_tmp(ind2:end,:))) = refpts_10_2p5_label_tmp(ind2:end);
...
for i = 1:length(frontal_line_left)-1
    ind1 = find(strcmpi(frontal_line_left(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(frontal_line_left(i+1),refpts_10_2p5_labels));
...
for i = 1:length(frontal_line_right)-1
    ind1 = find(strcmpi(frontal_line_right(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(frontal_line_right(i+1),refpts_10_2p5_labels));
...
for i = 1:length(Temp_edge_lower_left)
    ind1 = find(strcmpi(Temp_edge_lower_left(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Temp_edge_upper_left(i),refpts_10_2p5_labels));
...
for i = 1:length(Temp_edge_lower_right)
    ind1 = find(strcmpi(Temp_edge_lower_right(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Temp_edge_upper_right(i),refpts_10_2p5_labels));
...
for i = 1:length(Frontal_edge_left_lower)
    ind1 = find(strcmpi(Frontal_edge_left_lower(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Frontal_edge_left_upper(i),refpts_10_2p5_labels));
...
for i = 1:length(Frontal_edge_right_lower)
    ind1 = find(strcmpi(Frontal_edge_right_lower(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Frontal_edge_right_upper(i),refpts_10_2p5_labels));
...
for i = 1:length(Occ_edge_left_lower)
    ind1 = find(strcmpi(Occ_edge_left_lower(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Occ_edge_left_upper(i),refpts_10_2p5_labels));
...
for i = 1:length(Occ_edge_right_lower)
    ind1 = find(strcmpi(Occ_edge_right_lower(i),refpts_10_2p5_labels));
    ind2 = find(strcmpi(Occ_edge_right_upper(i),refpts_10_2p5_labels));
...
ind1 = find(strcmpi('Fp1hq',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp5',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp5';

ind1 = find(strcmpi('Fp1h',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp3',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp3';

ind1 = find(strcmpi('Fp1q',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp1',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp1';

ind1 = find(strcmpi('Fp2q',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp2',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp2';

ind1 = find(strcmpi('Fp2h',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp4',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp4';

ind1 = find(strcmpi('Fp2hq',refpts_10_2p5_labels));
ind2 = find(strcmpi('AFp6',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'FAFp6';

%Fill in occipital gaps manually
ind1 = find(strcmpi('O1q',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO5',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO5';

ind1 = find(strcmpi('O1h',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO3',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO3';

ind1 = find(strcmpi('O1hq',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO1',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO1';

ind1 = find(strcmpi('O2hq',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO2',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO2';

ind1 = find(strcmpi('O2h',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO4',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO4';

ind1 = find(strcmpi('O2q',refpts_10_2p5_labels));
ind2 = find(strcmpi('POO6',refpts_10_2p5_labels));
tmp = refpts_10_2p5(ind1,:) + (refpts_10_2p5(ind2,:)-refpts_10_2p5(ind1,:))./2;
qpoint = nearestNodeLocal(tmp,mshs.scalpSurfaceMeshUpsampled.node);
refpts_10_2p5(end+1,:) = qpoint;
refpts_10_2p5_labels{end+1} = 'POOO6';

% Save results in mshs? Dom't think I want this. mshs should be consistent
% with format definition.
...
function [nnode,ind,offset] = nearestNodeLocal(point,nodes)
